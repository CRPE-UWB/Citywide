...
data <- readRDS("DC_complete_analysis.Rdata")
str(data)
...
namelist <- c("MathschoolsStuckDC.csv", "MathschoolsStuckDetailsDC.csv", 
              "MathproficiencyDC.csv", "MathgainsDC.csv",
              "MathTopBottomDC.csv", "MathBTODC.csv",
              "MathStuckExtraDC.csv")

##### 2nd RUN = READ - - - - - 
# data$subject <- data$read

# namelist <- c("ELAschoolsStuckDC.csv", "ELAschoolsStuckDetailsDC.csv", 
#              "ELAproficiencyDC.csv", "ELAgainsDC.csv",
#              "ELATopBottomDC.csv", "ELABTODC.csv",
#              "ELAStuckExtraDC.csv")
...
lm1 <- lm(subject ~  propwhite + propblack + prophisp + propasian + factor(level) + enroll, data=data1)
lm2 <- lm(subject ~  propwhite + propblack + prophisp + propasian + factor(level) + enroll, data=data2)
lm3 <- lm(subject ~  propwhite + propblack + prophisp + propasian + factor(level) + enroll, data=data3)
lm4 <- lm(subject ~  propwhite + propblack + prophisp + propasian + factor(level) + enroll, data=data4)
summary(lm4)
...
mean(mdata$subject)
scaled1 <- scale(mdata$subject, center = TRUE, scale = FALSE)
scale(mdata$subject)
...
equation <- subject.std ~ time + propwhite + 
  propblack + prophisp + propasian + enroll
...
demo <- filter(demo, notschool != 1)
demo$schooltype[demo$level == 1 | demo$level == 2] <- "ElemMiddle"
demo$schooltype[demo$level == 3] <- "High"
demo$schooltype[demo$level == 4] <- "Other"


schools <- filter(demo, level == 4)
schools <- filter(schools, year == 2014)
schools <- select(schools, 1:3)
write.csv(schools, "dcAlternatives.csv")

unique(demo$year)
demo <- filter(demo, year == 2014)

city_demo <- ddply(demo, c("city.new", "schooltype"), summarise,
                   total_enroll = sum(enroll, na.rm = TRUE),
                   propwhite = mean(propwhite, na.rm = TRUE),
                   propblack = mean(propblack, na.rm = TRUE),
                   prophisp = mean(prophisp, na.rm = TRUE),
                   propother = mean(propasian, na.rm = TRUE))

city_demo <- ddply(demo, c("city.new"), summarise,
      total_enroll = sum(enroll, na.rm = TRUE),
      propwhite = mean(propwhite, na.rm = TRUE),
      propblack = mean(propblack, na.rm = TRUE),
      prophisp = mean(prophisp, na.rm = TRUE),
...
# Do they have a 12th grade
# Do they have a k
# Do they have 6

city_demo

write.csv(city_demo, "DCDemograpic.csv")

### (6) Math Enrollment ----------------------------------------------------------
ocr <- read.csv("/Users/Alton/OneDrive/Work/CRPE/City Wide/OCR/ocr_master.csv")
...
### - - - - - 
ocr <- filter(ocr, LEA_STATE == "DC")

ocr$NCES_SCHOOL_ID <- as.numeric(ocr$NCES_SCHOOL_ID)
names(ocr)[8] <- "ncessch"

### - - - - - 
ccd <- readRDS("/Users/Alton/OneDrive/Work/CRPE/City Wide/DC/ccd_2010_2015_DC.Rdata")
ccd <- readRDS("/Users/lualt/OneDrive/Work/CRPE/City Wide/DC/ccd_2010_2015_DC.Rdata")
...
write.csv(math, "MathEnrollmentDC.csv")



### (7) List of Schools in City and Charter proportion -------------------------------

# Going to want all unique schools using seasch in the city. 
# can pull name with it to ensure the schools are unique

school_list <- subset(city_df, !duplicated(seasch)) 
school_list <- select(school_list, 1:3, 16:21)

write.csv(school_list, "SchoolListDC.csv")

### Charter percentage
# Check which schools are charter
charter <- table(school_list$ischarter == "1" | school_list$ischarter == "Yes")
charter <- charter[2]/sum(charter)

write.csv(charter, "CharterProportionDC.csv")

school_list <- city_df
school_list <- filter(school_list, year == 2014)
unique(school_list$ischarter)
charter <- sum(filter(school_list, ischarter == "1" | ischarter == "Yes")$enroll)/sum(school_list$enroll)
charter
