...
setwd("/Users/Alton/OneDrive/Work/CRPE/City Wide/shapefiles/shp_DC") # if on Mac 
...
saveRDS(schools, "/Users/Alton/OneDrive/Work/CRPE/City Wide/DC/ccd_2010_2015_DC.Rdata")
...
data <- readRDS("DC_Complete.Rdata") # Provided from Alton
...
names(data)[3] <- "seasch" #school ID, might need to change to num
names(data)[1] <- "year"
names(data)[2] <- "stid"
#### - - - - - 
...
Alldata <- inner_join(data, ccd, by = c("seasch", "year", "stid"), match = "first")

# We want test to have a few observations as possible.
# If large amount, flag. 
test <- anti_join(data, ccd, by = c("seasch", "year", "stid"))
...
##### 
saveRDS(selectvars, "DC_complete_analysis.Rdata")
##### 
